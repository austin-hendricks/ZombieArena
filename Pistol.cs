...
namespace HendricksAustin.Lab6
{
    public class Pistol : MonoBehaviour
    {
        ...
        [SerializeField] private int magazineCapacity = 17;
        [SerializeField] private int loadedRounds = 17;
        [SerializeField] private int initialAmmoReserve = 68;
        ...
        [SerializeField] private float timeBetweenShots = 0.051f;

        [Header("Recoil Strength")]
        [SerializeField] private float muzzleFlipAngle = 1.5f;
        [SerializeField] private float recoilResetTime = 0.05f;
        ...
        private readonly Vector3 gunPositionAim = new(-0.28f, 0.07f, 0f);
        private readonly Vector3 gunPositionReload = new(0.7f, -0.05f, 0.15f);
        private readonly Quaternion gunRotationReload = Quaternion.Euler(-10f, -90f, 0f);
        private readonly Vector3 gunPositionHipRecoil = new(0f, 0f, -0.05f);
        private readonly Vector3 gunPositionAimRecoil = new(-0.28f, 0.07f, -0.05f);
        private readonly float hipFOV = 60f;
        private readonly float aimFOV = 45f;
        ...
        public void Shoot()
        {
            if (readyToShoot && !reloading && loadedRounds > 0)
            {
                ...
                currentCasing.GetComponentInChildren<Rigidbody>().AddForce(cameraTransform.right * casingEjectionForce, ForceMode.Impulse);
                currentCasing.GetComponentInChildren<Rigidbody>().AddForce(cameraTransform.up * casingEjectionUpwardForce, ForceMode.Impulse);
